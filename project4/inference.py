...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # print "Position ", pacmanPosition
        # print "legalpos ", self.legalPositions

        # New beliefsChart
        allPossible = util.Counter()

        # Handling The Edge case
        if None == noisyDistance:
            # Only one item "Jail".  With a probability == 1
            allPossible[self.getJailPosition()] = 1.0
        else:
            for itemPos in self.legalPositions:
                dist = util.manhattanDistance(itemPos, pacmanPosition)
                # Attention, emissionModel is already taken nosiy = k intoconsideration
                # P(g@Pos | noisy=k ) = P(noisy = k | g@Pos) * P(g@Pos) / P(noisy = k)
                # P(g@Pos | noisy=k ) = alpha * emissionModel(k, g@pos) * p(g@Pos)
                allPossible[itemPos] = emissionModel[dist] * self.beliefs[itemPos]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        # Check Every possible postion in current circumstances
        for oldPos in self.legalPositions:
            #set a mimicGameState as if ghost locates @ oldPos
            mimicGameState = self.setGhostPosition(gameState, oldPos)
            # Get the Distribution
            newPosDist = self.getPositionDistribution(mimicGameState)
            #print "newPostDist ", newPosDist
            for newPos, prob in newPosDist.items():
                allPossible[newPos] += prob * self.beliefs[oldPos]
        # Update at last / cannot update in loop, since we need old self.beliefs
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        ParticlePerPosition = 0
        legalPosNum = len(self.legalPositions)
        ParticlePerPosition = self.numParticles / legalPosNum
        RemainNum = self.numParticles - ParticlePerPosition * legalPosNum
        print "ParticalPerPositon = ", ParticlePerPosition
        print "Remain = ", RemainNum

        self.particles = []
        for index in range(legalPosNum):
            if index < RemainNum:
                poslist = [self.legalPositions[index] for i in range(ParticlePerPosition + 1)]
            else:
                poslist = [self.legalPositions[index] for i in range(ParticlePerPosition)]
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        # If That Ghost is captured
        if None == noisyDistance:
            self.particles = [self.getJailPosition()] * self.numParticles
            return

        # If That Ghost is not captured
        # Transmission: Regenerate A new Distribution/Weight base on evidence
        # emission model
        weightPos = util.Counter()
        oldProb = self.getBeliefDistribution()
        # N(x_t+1 | e_1:t) = sum_x_t{P(x_t+1 | x_t) x N(x_t | e_1:t)}
        # Very Tricky...
        for itemPos in self.legalPositions:
            dist = util.manhattanDistance(itemPos, pacmanPosition)
            weightPos[itemPos] += emissionModel[dist] * oldProb[itemPos]

        # If receive 0 weight
        if not any(weightPos.values()):
            self.initializeUniformly(gameState)
            return

        resampleList = []
        for index in range(0, self.numParticles):
            # Using the weight to regenerate
            newSample = util.sample(weightPos)
            resampleList.append(newSample)
        self.particles = resampleList
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            mimicGamestate = self.setGhostPosition(gameState, oldPos)
            newPosDistribution = self.getPositionDistribution(mimicGamestate)
            newSample = util.sample(newPosDistribution)
            resampleList.append(newSample)
        self.particles = resampleList
    ...
    def getBeliefDistribution(self):
        ...
        # Classification and Normalized
        for itemPos in self.particles:
            distribution[itemPos] += 1
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for index in range(legalPosNum):
            if index < RemainNum:
                poslist = [possPositions[index] for i in range(ParticlePerPosition + 1)]
            else:
                poslist = [possPositions[index] for i in range(ParticlePerPosition)]
            self.particles += poslist
    ...
    def observeState(self, gameState):
        ...
        for itemParticle in self.particles:
            part = 1.0
            for index in range(self.numGhosts):
                if None == noisyDistances[index]:
                    itemParticle = self.getParticleWithGhostInJail(itemParticle, index)
                else:
                    dist = util.manhattanDistance(pacmanPosition, itemParticle[index])
                    # Posterior Possibility
                    part *= emissionModels[index][dist]

            allPossible[itemParticle] += part
        ...
        for index in range(self.numParticles):
            newSample = util.sample(allPossible)
            particleList.append(newSample)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for index in range(self.numGhosts):
                mimicGameState = setGhostPositions(gameState, newParticle)
                thisGhostAgent = self.ghostAgents[index]
                newPosDistForGhost = getPositionDistributionForGhost(mimicGameState, index, thisGhostAgent)
                newParticle[index] = util.sample(newPosDistForGhost)
